...
(function () {
  ...
  */
  ...
  /* ======================================================================
     ...
     O cameră pe ecran: firul de mesaje sus, caseta de scris jos. Mesajele
     sosite se cer din când în când (api/chat-mesaje.php) și vin GATA
     DESENATE, din aceleași funcții PHP care au scris pagina — aici doar se
     lipesc la coada listei. Nicio bulă nu se construiește în JS: ar fi fost a
     doua descriere a aceluiași lucru, în alt limbaj, și text venit de la om
     lipit în pagină fără trecerea prin h().
     ====================================================================== */

  var chatPanou = document.querySelector('[data-chat]');

  if (chatPanou) {
    var chatFir    = chatPanou.querySelector('[data-chat-fir]');
    var chatForma  = chatPanou.querySelector('[data-chat-forma]');
    var chatCamp   = chatPanou.querySelector('[data-chat-camp]');
    var chatButon  = chatPanou.querySelector('[data-chat-trimite]');
    var chatGol    = chatPanou.querySelector('[data-chat-gol]');
    var chatEroare = chatPanou.querySelector('[data-chat-eroare]');

    var chatCamera  = chatPanou.getAttribute('data-camera') || 'general';
    var chatAdresa  = chatPanou.getAttribute('data-camera-adresa') || '';
    var chatUltimId = parseInt(chatPanou.getAttribute('data-ultim-id'), 10) || 0;
    var chatMoment  = chatPanou.getAttribute('data-moment') || '';

    var chatCsrf = (chatForma.querySelector('[name="csrf"]') || {}).value || '';

    /* Din câte în câte milisecunde se întreabă ce s-a mai spus.
       Patru secunde: destul de des cât discuția să pară vie, destul de rar cât
       o cameră cu treizeci de oameni în ea să nu însemne treizeci de cereri pe
       secundă. Nu e WebSocket, și n-are de ce să fie: aici se vorbește în
       oraș, nu se joacă șah pe timp. */
    var CHAT_PAUZA = 4000;

    var chatCeas    = null;
    var chatInZbor  = false;   // o întrebare e deja pe drum
    var chatTrimite = false;   // un mesaj e deja pe drum

    /* --- Fundul firului ---------------------------------------------------
       Se coboară automat numai dacă omul ERA deja jos. Cine s-a urcat să
       recitească ceva de acum zece minute n-are de ce să fie smucit înapoi la
       fiecare mesaj nou al altcuiva.

       Cele 40 de puncte de îngăduință sunt pentru derularea „aproape jos":
       degetul pe telefon rareori oprește fix la capăt.                     */
    function chatEJos() {
      return chatFir.scrollHeight - chatFir.scrollTop - chatFir.clientHeight < 40;
    }

    function chatLaFund() {
      chatFir.scrollTop = chatFir.scrollHeight;
    }

    /* --- Ce n-a mers ---------------------------------------------------- */
    function chatSpune(text) {
      if (!chatEroare) return;

      if (!text) {
        chatEroare.hidden = true;
        chatEroare.textContent = '';
        return;
      }

      chatEroare.textContent = text;
      chatEroare.hidden = false;
    }

    /* --- Lipirea mesajelor sosite ----------------------------------------
       HTML-ul vine de la server. Se trece printr-un <template>, nu prin
       innerHTML pe listă: altfel browserul ar fi aruncat `<li>`-urile care nu
       stau (încă) într-o listă, iar la a doua lipire s-ar fi rescris toate
       bulele de dinainte — adică s-ar fi pierdut selecția omului care tocmai
       copia un text.

       Mesajele deja pe ecran se sar. Cel scris de mine ajunge aici de două
       ori: o dată din răspunsul la trimitere, o dată din prima întrebare de
       după — fără verificarea asta l-aș fi văzut dublu.                    */
    function chatAdauga(html) {
      if (!html) return 0;

      var cutie = document.createElement('template');
      cutie.innerHTML = html;

      var noi = cutie.content.querySelectorAll('[data-mesaj]');
      var puse = 0;

      for (var i = 0; i < noi.length; i++) {
        var id = noi[i].getAttribute('data-mesaj');

        if (chatFir.querySelector('[data-mesaj="' + id + '"]')) { continue; }

        chatFir.appendChild(noi[i]);
        puse++;
      }

      if (puse && chatGol) { chatGol.hidden = true; }

      return puse;
    }

    /* --- Scoaterea celor șterse ------------------------------------------
       Un mesaj șters nu mai vine în niciun răspuns cu mesaje, deci cine îl
       avea pe ecran l-ar fi ținut acolo până la prima reîncărcare — adică
       exact omul de la care trebuia să dispară. De aceea serverul trimite
       separat id-urile stinse (vezi mesajeSterseDupa în inc/chat.php).     */
    function chatScoate(ids) {
      if (!ids || !ids.length) return;

      for (var i = 0; i < ids.length; i++) {
        var bula = chatFir.querySelector('[data-mesaj="' + ids[i] + '"]');
        if (bula) { bula.remove(); }
      }

      if (chatGol && !chatFir.querySelector('[data-mesaj]')) { chatGol.hidden = false; }
    }

    /* --- Întrebarea: ce s-a mai spus? ------------------------------------ */
    function chatIntreaba() {
      if (chatInZbor || document.hidden) return;

      chatInZbor = true;

      var eJos = chatEJos();

      var adresa = 'api/chat-mesaje.php?camera=' + encodeURIComponent(chatAdresa)
                 + '&dupa=' + chatUltimId
                 + '&sters_dupa=' + encodeURIComponent(chatMoment);

      fetch(adresa, { credentials: 'same-origin' })
        .then(citesteRaspuns)
        .then(function (rez) {
          chatInZbor = false;

          if (!rez.corp) return;          // o eroare de server: se încearcă din nou
          var c = rez.corp;

          /* Sesiunea s-a stins cât stătea fila deschisă. Fără oprire, fila ar
             fi bătut la ușă la fiecare patru secunde, degeaba, până seara. */
          if (rez.stare === 401) {
            chatOpreste();
            chatSpune('Sesiunea a expirat. Reîncarcă pagina ca să vezi discuția.');
            return;
          }

          if (!c.ok) return;

          /* Răspunsul e din altă cameră decât cea de pe ecran: omul a schimbat
             camera exact cât zbura întrebarea. Nu e al lui, se aruncă. */
          if (c.camera && c.camera !== chatCamera) return;

          chatScoate(c.sterse);

          var puse = chatAdauga(c.html);

          if (typeof c.ultim_id === 'number' && c.ultim_id > chatUltimId) {
            chatUltimId = c.ultim_id;
          }

          if (c.moment) { chatMoment = c.moment; }

          if (puse && eJos) { chatLaFund(); }
        })
        .catch(function () {
          // Legătura a căzut. Nu se spune nimic pe ecran: la următoarea
          // trecere fie merge, fie tot nu — un rând roșu care apare și dispare
          // la fiecare patru secunde e mai supărător decât lipsa mesajelor.
          chatInZbor = false;
        });
    }

    function chatPorneste() {
      if (chatCeas) return;
      chatCeas = setInterval(chatIntreaba, CHAT_PAUZA);
    }

    function chatOpreste() {
      clearInterval(chatCeas);
      chatCeas = null;
    }

    /* Fila ascunsă nu întreabă nimic: nu se uită nimeni la ea, iar pe telefon
       ar fi ținut radioul pornit degeaba. La întoarcere se întreabă pe loc, ca
       omul să nu aștepte patru secunde ca să vadă ce a pierdut. */
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) { chatOpreste(); return; }
    ...
    /* --- Caseta care crește ----------------------------------------------
       Un rând cât e o vorbă scurtă — adică aproape mereu. Cine scrie un
       paragraf îl vede întreg, până la max-height din CSS, de unde încolo se
       derulează în casetă.                                                 */
    function chatCreste() {
      chatCamp.style.height = 'auto';
      chatCamp.style.height = chatCamp.scrollHeight + 'px';
    }

    chatCamp.addEventListener('input', function () {
      chatCreste();
      chatSpune('');
    });

    /* Enter trimite, Shift+Enter trece la rândul următor.
       `isComposing` e pentru tastaturile care compun caracterul din mai multe
       apăsări: acolo primul Enter încheie cuvântul, nu mesajul. */
    chatCamp.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
        e.preventDefault();
        chatForma.requestSubmit ? chatForma.requestSubmit() : chatForma.submit();
      }
    });

    /* --- Trimiterea ------------------------------------------------------- */
    chatForma.addEventListener('submit', function (e) {
      e.preventDefault();

      if (chatTrimite) return;

      var text = chatCamp.value.trim();

      if (!text) { chatCamp.focus(); return; }

      chatTrimite = true;
      if (chatButon) { chatButon.disabled = true; }
      ...
      .then(function (rez) {
        chatTrimite = false;
        if (chatButon) { chatButon.disabled = false; }

        if (!rez.corp) { chatSpune(mesajRaspunsNeasteptat(rez)); return; }
        var c = rez.corp;

        if (rez.stare === 401) {
          chatSpune('Intră în cont ca să scrii în chat.');
          setTimeout(goToLogin, 900);
          return;
        }

        if (!c.ok) {
          /* Mesajul RĂMÂNE în casetă la orice fel de refuz. Omul l-a scris, e
             al lui: la „mai încet" trebuie doar să mai apese o dată peste
             câteva secunde, iar la „prea lung" are de tăiat din el — și
             n-are ce tăia dintr-o casetă golită. */
          chatSpune(c.mesaj || 'Mesajul nu a plecat.');
          return;
        }
        ...
        if (typeof c.id === 'number' && c.id > chatUltimId) { chatUltimId = c.id; }

        // La mesajul MEU se coboară mereu, chiar dacă citeam mai sus: eu l-am
        // scris, deci vreau să-l văd plecat.
        chatLaFund();
        chatCamp.focus();
      })
      .catch(function () {
        chatTrimite = false;
        if (chatButon) { chatButon.disabled = false; }
        chatSpune(mesajFaraLegatura());
      });
    });

    /* --- „×"-ul de ștergere (numai staff) --------------------------------
       Apăsat o dată, întreabă; apăsat a doua oară, șterge. Fără fereastra de
       sistem a browserului — sare peste toată pagina, cu alte litere și alte
       butoane decât restul site-ului — și fără o casetă nouă de confirmare
       lângă fiecare bulă, care ar fi împins discuția în jos.

       Ștergerea nu se ia înapoi, deci o apăsare greșită trebuie să aibă unde
       să se oprească. A doua apăsare vine de la un deget care știe ce face.

       Ascultătorul stă pe listă, nu pe fiecare buton: bulele apar întruna, iar
       unul pus pe fiecare ar fi trebuit legat din nou la fiecare mesaj sosit.  */
    var chatDeSters = null;

    function chatUita() {
      if (!chatDeSters) return;

      chatDeSters.classList.remove('is-confirma');
      chatDeSters.setAttribute('title', 'Șterge mesajul');
      chatDeSters = null;
    }

    chatFir.addEventListener('click', function (e) {
      var buton = e.target.closest('[data-sterge-mesaj]');

      // Apăsat altundeva: întrebarea rămasă în aer se stinge.
      if (!buton) { chatUita(); return; }

      if (buton !== chatDeSters) {
        chatUita();

        chatDeSters = buton;
        buton.classList.add('is-confirma');
        buton.setAttribute('title', 'Mai apasă o dată ca să ștergi');
        return;
      }

      var id = parseInt(buton.getAttribute('data-sterge-mesaj'), 10) || 0;
      var bula = buton.closest('[data-mesaj]');
      ...
      .then(citesteRaspuns)
      .then(function (rez) {
        if (!rez.corp) { buton.disabled = false; toast(mesajRaspunsNeasteptat(rez)); return; }
        var c = rez.corp;

        if (!c.ok) { buton.disabled = false; toast(c.mesaj || 'Mesajul nu s-a șters.'); return; }

        if (bula) { bula.remove(); }

        if (chatGol && !chatFir.querySelector('[data-mesaj]')) { chatGol.hidden = false; }

        toast('Mesajul a fost șters.');
      })
      .catch(function () {
        buton.disabled = false;
        toast(mesajFaraLegatura());
    ...
    // Escape stinge întrebarea, ca peste tot pe site.
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape') { chatUita(); }
    });
